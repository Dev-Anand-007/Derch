...
import { fetchAllProductUser } from "../../Admin/AdminAPI";
import { addtoCart } from "../HomePageAPI";
...
const NewItems = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetchAllProductUser();
        console.log(res.products)
        // filter only products with isNewProduct === true
      const newProducts = res.products.filter(p => p.isNewProduct === true);
          ...
          {products.length == 0 ? (
            <div className="text-center py-12">
              ...
              <h2
                ...
                style={{ color: mode.text }}
              >
                Admin didn't uploaded any item
              </h2>
              <p style={{ color: mode.subText }}>
                Wait till admin upload any item
              </p>
            </div>
          ) : (
            <>
